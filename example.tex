...
% Controls whether heading numbers are printed. The default 2 numbers chapters,
% sections and subsections; use -1 to suppress all heading numbers.
\setcounter{secnumdepth}{2} % May be changed if fewer heading levels are desired.
...
\begin{declaration}
The declaration in a submitted thesis must accurately describe the candidate's original contribution and any material completed jointly with others. Replace this instructional paragraph with the declaration agreed for the thesis; do not retain template text in the submitted document.
\end{declaration}
...
\begin{figure}[htbp]
  ...
  \caption{The University of Liverpool campus and the Victoria Building.}
  \label{fig:campus}
\end{figure}
...
\begin{table}[htbp]
  \centering
  \caption{Principal files and their roles.}
  \label{tab:files}
...
\setcounter{secnumdepth}{2} % Use -1 to suppress all heading numbers.
\end{lstlisting}

For double-sided printing, replace \texttt{oneside} with \texttt{twoside}. The default paragraph style uses first-line indentation. Add the class option \texttt{parskip} if separated paragraphs are preferred consistently throughout the document.
...
The abstract environment is single-spaced automatically. Annexe 1 limits it to one side of A4, normally about 450 words, and requires it to state the aims and results of the investigation \citep{uolformatting}. The other preliminary sections inherit the normal thesis spacing.
...
The preamble setting \verb|\setcounter{secnumdepth}{2}| prints numbers on chapters, sections and subsections. Thus \verb|\chapter{Introduction}| begins with ``1. Introduction''. Use 1 to stop numbering after sections, 0 to number chapters only, or $-1$ to suppress all heading numbers.
...
Use ordinary \texttt{table} and \texttt{tabular} environments. Captions appear above tables automatically. The \texttt{booktabs} rules produce a clear journal-style table without vertical lines, as demonstrated in Table~\ref{tab:comparison}.

\begin{table}[htbp]
  \centering
  \caption{Example comparison using ordinary table syntax.}
...
Store bibliographic data in \texttt{references.bib}. Cite sources with commands such as \verb|\citep{lamport1994}| or \verb|\citet{lamport1994}|; the resulting entries are formatted consistently rather than typed manually \citep{lamport1994}.

\section{Printing the reference list}

The following command creates a numbered-free References chapter and includes it in the table of contents:
