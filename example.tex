...
For double-sided printing, select the \texttt{twoside}, \texttt{frontstart=right} and \texttt{header=outer} class options. The default \texttt{frontstart=right} places Roman page i on a physical right-hand page after an unnumbered blank reverse of the cover; use \texttt{frontstart=left} to place page i immediately behind the cover on a physical left-hand page. The alternatives \texttt{header=left}, \texttt{header=right}, \texttt{header=inner} and \texttt{header=outer} select a fixed left position, a fixed right position, the physical binding edge and the physical outer edge respectively. The default \texttt{header=auto} chooses right for \texttt{oneside} and outer for \texttt{twoside}. The same header selection can be made in the preamble with, for example, \verb|\thesisheaderstyle{inner}|. Mirrored margins, alternating headers and inserted blank pages follow physical PDF-page parity even when printed page numbers restart; chapter-opening and blank pages remain header-free.

\section{Front matter}

Place \verb|\frontmatter| before \verb|\maketitle|. The cover is unnumbered and is not counted as Roman page i; the first preliminary heading after it is page i, followed by lower-case Roman numbering throughout the front matter. In double-sided output, \texttt{frontstart=right} or \texttt{frontstart=left} controls the physical side on which page i begins. Annexe 1 requires a title page, an abstract, and a table of contents containing chapter headings, page numbers and every separate section of the thesis. The demonstration additionally includes the following useful preliminary sections where applicable:
